...
function aimTable(players: PlayerScoreAggregate[], focusIds?: Set<string>): string {
  if (players.length === 0) {
  ...
  const rows = players.map((p) => {
    ...
    return (
      `| ${name} | ${p.avgOverallScore} | ${p.avgAimScore} | ${a.accuracy}% | ${a.headAccuracy}% | ` +
...
function utilityTable(players: PlayerScoreAggregate[], focusIds?: Set<string>): string {
  if (players.length === 0) {
  ...
  const rows = players.map((p) => {
    ...
    const u = p.utility;
    return (
      `| ${name} | ${p.avgUtilityScore} | ${u.flashesThrown} | ${u.effectiveFlashPct}% | ${u.enemiesFlashedPct}% | ` +
...
function positioningTable(players: PlayerScoreAggregate[], focusIds?: Set<string>): string {
  if (players.length === 0) {
  ...
  const rows = players.map((p) => {
    ...
    return (
      `| ${name} | ${p.avgPositioningScore} | ${pos.openingDuelWinPct}% | ${pos.openingDuelParticipationPct}% | ` +
