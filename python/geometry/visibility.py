"""Module for calculating line-of-sight visibility against CS2 map collision geometry.

Adapted from awpy (https://github.com/pnxenopoulos/awpy), MIT licensed —
see THIRD_PARTY_NOTICE.md in this directory. awpy in turn credits
https://github.com/AtomicBool/cs2-map-parser as the reference for the
.vphys parsing approach.

Changes from upstream: uses this package's own Vector3 (python/geometry/vector.py)
instead of awpy.vector, and stdlib `logging` instead of `loguru`. Parsing/BVH/
raycasting logic is otherwise unchanged.
"""
...
logger = logging.getLogger("cs_demo_analyst.geometry")
...
class KV3Parser:
    ...
    def _skip_until_first_bracket(self) -> None:
        ...
        Deviation from upstream awpy: guards against content with no more
        newlines before the next "{" (or none at all) — upstream's
        `self.index = self.content.find(...) + 1` wraps back to 0 forever
        when `find` returns -1, an infinite loop on malformed/truncated
        input. A demo-analysis tool must never hang on a corrupted
        extraction, so this advances to end-of-content and lets the caller
        (get_value returning "" for everything) fail gracefully instead.
        """
...
class VphysParser:
    ...
    def bytes_to_vec(byte_str: str, element_type: Literal["uint8", "int32", "float"]) -> list[int] | list[float]:
        ...
        Args:
            ...
            element_type (str): Type represented by the bytes (uint8, int32, float).
    ...
    def parse(self) -> None:
        ...
        if len(self.triangles) > 0:
            logger.debug("VPhys data already parsed, got %d triangles.", len(self.triangles))
            return

        logger.debug("Parsing vphys file: %s", self.vphys_file)
        ...
        logger.debug("Extracted collision attribute indices: %s", collision_attribute_indices)
        ...
        while True:
            if hull_idx % 1000 == 0:
                logger.debug("Processing hull %d...", hull_idx)
        ...
        mesh_count = 0
        while True:
            logger.debug("Processing mesh %d...", mesh_idx)
            collision_idx = self.kv3_parser.get_value(
    ...
    def to_tri(self, path: str | pathlib.Path | None) -> None:
        ...
        logger.debug("Exporting %d triangles to %s", len(self.triangles), outpath)
        ...
        logger.info("Processed %d triangles from %s -> %s", len(self.triangles), self.vphys_file, outpath)
...
class VisibilityChecker:
    ...
    def _ray_triangle_intersection(
        ...
        """Check if a ray intersects with a triangle (Moller-Trumbore algorithm).
    ...
    def is_visible(
        ...
        """Check if a line segment is unobstructed (no geometry between the two points).
        ...
        Returns:
            bool: True if the line segment is unobstructed, False otherwise.
        """
